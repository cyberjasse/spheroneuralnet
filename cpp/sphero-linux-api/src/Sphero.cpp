...
void Sphero::notifyStream(struct StreamFrame *frame){
	streamobservers[0].notify(frame);
...
void Sphero::setRawMotorValues(int16_t lpower, int16_t rpower, uint8_t lmode, uint8_t rmode){
	uint8_t lp, rp;
	if(lpower < 0){
		lpower = -lpower;
		lmode  = 0x02;
	...
	uint8_t data_payload[4];
	/* About mode:
		0x00 : motor off. open circuit
		0x01 : Forward
		0x02 : reverse
		0x03 : Brake. motor shorted
		0x04 : Ignore. Doesn't change motor mode and power.
	All modes instead of 0x04 disable stabilization. Need to re-able via CID 0x02.*/
	...
	ClientCommandPacket packet = ClientCommandPacket(
				...
				flags::notNeeded,
				0x05,//DLEN following api-reference
				data_payload,
				_waitConfirm,
				_resetTimer);
	sendPacket(packet);
